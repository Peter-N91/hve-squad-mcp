# Architecture Advisor — connected agent description
...
Do not use when there is no evidence yet (use Research Advisor first), when the architecture is settled and the user wants a work breakdown (use Delivery Planner), or when an existing design must be judged against acceptance criteria (use Quality Reviewer).
