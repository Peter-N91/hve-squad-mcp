...
test("SEC-1: original request metadata reaches the configured token verifier", async () => {
  const verifier = new FakeJwtVerifier();
  const { handler } = buildHarness({ verifier });
  verifier.register({
    token: "request-metadata",
    tenantId: TENANT,
    subject: "user-metadata-oid",
    scopes: ["Squad.Research"],
  });
  ...
  const res = await handler.handle({
    ...
    headers: {
      origin: ORIGIN,
      authorization: bearer("request-metadata"),
      ...JSON_HEADERS,
    },
    tokenValidation,
    body: { jsonrpc: "2.0", id: 1, method: "initialize", params: {} },
...
test("SEC-1: a forged/unverifiable token is rejected 401 (never trusted)", async () => {
  const { handler, backend, logger } = buildHarness(); // fresh verifier knows no tokens
  const res = await handler.handle({
    method: "POST",
    path: "/mcp",
  ...
  assert.equal(
    logger.secretSet.has("forged.jwt.value"),
    false,
    "invalid attacker input must not enter the redaction set",
  );
  assert.equal(backend.callCount, 0);
});
...
test("SEC-2: tools/list hides enabled tools the caller is not scoped to invoke", async () => {
  const verifier = new FakeJwtVerifier();
  const harness = buildHarness({ verifier });
  verifier.register({
    token: "research-only-list",
    tenantId: TENANT,
    subject: "user-c-oid",
    scopes: ["Squad.Research"],
  });
  const sessionId = await initializeSession(harness.handler, "research-only-list");
  const res = await harness.handler.handle({
    method: "POST",
    path: "/mcp",
    headers: {
      origin: ORIGIN,
      authorization: bearer("research-only-list"),
  ...
  const tools = (res.body as { result?: { tools?: { name: string }[] } }).result?.tools ?? [];
  assert.deepEqual(tools.map((tool) => tool.name), ["squad_research"]);
});
